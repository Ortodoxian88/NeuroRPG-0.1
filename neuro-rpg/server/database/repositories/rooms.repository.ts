...
export const roomsRepository = {
  ...
  async findById(id: string): Promise<RoomRow | null> {
    const res = await query<RoomRow>('SELECT * FROM rooms WHERE id = $1 OR join_code = $1', [id]);
    return res.rows[0] || null;
  },

  async findByJoinCode(joinCode: string): Promise<RoomRow | null> {
    const res = await query<RoomRow>('SELECT * FROM rooms WHERE join_code = $1 OR id = $1', [joinCode]);
    return res.rows[0] || null;
  },
